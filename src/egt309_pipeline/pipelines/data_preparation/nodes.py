...
def _my_knnimputer(
    ...
    if target_val is not None:
        df_copy[target_col] = df_copy[target_col].map(lambda x: np.nan if x==target_val else x)
...
def clean_contactMethod(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df_new = df.copy()
    df_new["Contact Method"] = df_new["Contact Method"].map(lambda x:"cellular" if x[0].lower()=="c" else "telephone")
    return df_new
...
def clean_previousContactDays(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_new["Previous Contact Days"] = df_new["Previous Contact Days"].map(lambda x: -1 if x == 999 else x)
...
def clean_subscriptionStatus(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df_new = df.copy()
    df_new["Subscription Status"] = df_new["Subscription Status"].map(lambda x: True if x=="yes" else False)
    return df_new
