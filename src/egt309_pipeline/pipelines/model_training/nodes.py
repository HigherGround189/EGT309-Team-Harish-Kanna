...
def _parse_search_space(search_space: dict) -> Dict[str, Any]:
    ...
    for identifier, values in search_space.items():
        ...
        if value_type == 'Integer':
            ...
            bayes_search_params[identifier] = Real(values['low'], values['high'], **kwargs)
...
def train_model(X_train, y_train, model_config: dict, options: dict):
    ...
    cv_strategy = StratifiedKFold(n_splits=options["cv_splits"], shuffle=True, random_state=options["random_state"])
