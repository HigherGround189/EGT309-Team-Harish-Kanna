...
def plot_subscription_distribution_categorical(df, col_name, distribution_col="Subscription Status", debug=False):
    df = df.with_columns(
        pl.all().exclude(pl.Int64, pl.String).cast(pl.String)
    )
    cprint(df)
    ...
    long_counts = counts.sort(by=unique_values).melt(col_name).join(
        totals,
        on=col_name,
        how="left"
    ).with_columns(
    ...
    fig = px.bar(
        ...
        color_discrete_map={ # Hardcoded colour values cause jun hoe has an extreme reaction to the regular one
            "no": "rgb(239, 85, 59)", "False": "rgb(239, 85, 59)", "false": "rgb(239, 85, 59)",
            "yes": "rgb(99, 110, 251)", "True": "rgb(99, 110, 251)", "true": "rgb(99, 110, 251)"
        }
    )
...
def plot_distribution(df, col_name, distribution_col="Subscription Status", debug=False):
    ...
    column = df[col_name]
    if column.dtype == pl.Categorical or column.dtype == pl.String or column.dtype == pl.Boolean: # Added pl.Boolean here
        plot_categorical(df, col_name, debug=debug)

        if col_name != distribution_col:
            plot_subscription_distribution_categorical(df, col_name, distribution_col, debug=debug)
